...
function rowToTag(row: TagRow): Tag {
  return {
    ...
    synonyms: jsonArrayFromColumn<string>(row.synonyms),
    colour: row.colour,
    icon: row.icon,
...
export class TagsRepository {
  ...
  async create(input: TagCreateInput): Promise<Tag> {
    ...
    await this.db.run(
      ...
      [
        ...
        jsonArrayToColumn(input.synonyms),
        input.colour,
        input.icon,
        now,
        now,
  ...
  async update(tag: string, patch: TagUpdateInput): Promise<void> {
    ...
    if (patch.synonyms !== undefined)    { sets.push('synonyms = ?');    params.push(jsonArrayToColumn(patch.synonyms)) }
